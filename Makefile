...
ifeq ($(OS),Windows_NT)
		set -e; for dir in $(ALL_GO_MOD_DIRS); do \
		echo "go tool github.com/onsi/ginkgo/v2/ginkgo $${dir}/..."; \
		(cd "$${dir}" && \
		export CGO_ENABLED=0; go tool github.com/onsi/ginkgo/v2/ginkgo --output-dir=. ./...); \
		done; 
else
		set -e; for dir in $(ALL_GO_MOD_DIRS); do \
		echo "go tool github.com/onsi/ginkgo/v2/ginkgo $${dir}/..."; \
		(cd "$${dir}" && \
		export CGO_ENABLED=1; go tool github.com/onsi/ginkgo/v2/ginkgo -race --output-dir=. ./...); \
		done;
endif
...
vuln: install_tools $(ADDITIONAL_CI_STEPS)
	set -e; for dir in $(ALL_GO_MOD_DIRS); do \
		echo "go tool  golang.org/x/vuln/cmd/govulncheck  $${dir}/..."; \
		(cd "$${dir}" && \
